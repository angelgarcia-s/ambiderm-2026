<x-layouts.public title="Productos Ambiderm - Calidad y Protección">
        ...
        <div class="max-w-[1024px] mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-4">
            <div class="flex gap-2 md:gap-4 overflow-x-auto min-w-max pb-2 md:pb-0 scrollbar-hide">
                <a href="{{ route('productos') }}"
                    class="category-btn px-6 py-2 rounded-full font-bold text-xs md:text-sm tracking-wide transition-all {{ !request('categoria') ? 'bg-brand-ink text-white shadow-lg' : 'bg-gray-100 text-gray-500 hover:bg-gray-200 hover:text-gray-900' }}">TODOS</a>
                @foreach ($categorias as $cat)
                    <a href="{{ route('productos', ['categoria' => $cat->slug]) }}"
                        class="category-btn px-6 py-2 rounded-full font-bold text-xs md:text-sm tracking-wide transition-all {{ request('categoria') === $cat->slug ? 'bg-brand-ink text-white shadow-lg' : 'bg-gray-100 text-gray-500 hover:bg-gray-200 hover:text-gray-900' }}">{{ strtoupper($cat->nombre) }}</a>
                @endforeach
            </div>
        ...
        <div class="max-w-[1240px] mx-auto">
            <div id="products-grid" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 md:gap-10">
                ...
                @forelse ($productos as $producto)
                    <div class="group product-card-hover cursor-pointer reveal reveal-fade-in"
                         data-category="{{ $producto->categorias->first()?->slug ?? '' }}"
                         data-name="{{ strtolower($producto->nombre) }}"
                         data-material="{{ strtolower($producto->material ?? '') }}">
                        <a href="{{ route('producto.detalle', $producto->slug) }}">
                            ...
                            <div class="text-center">
                                <span class="text-[10px] font-bold text-brand-blue uppercase tracking-widest mb-1 block">{{ $producto->material ?? $producto->categorias->first()?->nombre }}</span>
                                <h3 class="text-lg md:text-xl font-bold text-brand-ink mb-2">{{ $producto->nombre }}</h3>
                                <span class="inline-flex items-center text-sm font-semibold text-gray-400 group-hover:text-brand-blue transition-colors">
                ...
                @empty
                    <div class="col-span-full text-center py-20">
                        ...
                        <p class="text-xl font-bold text-gray-400">No hay productos disponibles</p>
                        <p class="text-gray-400 mt-2">Vuelve pronto para ver nuestro catálogo actualizado.</p>
                    </div>
                @endforelse
    ...
    <x-slot:scripts>
        ...
        <script>
            ...
            // PRODUCT SEARCH (client-side text filter)
            const searchInput = document.getElementById('product-search');
            if (searchInput) {
                searchInput.addEventListener('input', (e) => {
                    const term = e.target.value.toLowerCase().trim();
                    document.querySelectorAll('#products-grid > div[data-name]').forEach(card => {
                        const name     = card.dataset.name ?? '';
                        const material = card.dataset.material ?? '';
                        card.classList.toggle('hidden', term !== '' && !name.includes(term) && !material.includes(term));
                    });
                });
            }
        </script>
