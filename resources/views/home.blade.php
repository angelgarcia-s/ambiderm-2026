<x-layouts.public title="Ambiderm - Innovación en Protección" :nav-transparent="true">
    ...
    <div class="max-w-[1200px] mx-auto relative z-10">
      <div class="reveal reveal-fade-in" style="transition-delay: 200ms;">
        <h1 class="text-6xl md:text-8xl lg:text-9xl font-black tracking-tighter mb-6">
          <span class="bg-clip-text text-transparent bg-gradient-to-r from-blue-600 via-blue-400 to-cyan-400">{{ $hero['titulo'] }}</span>
        </h1>
      </div>
        ...
        <div class="md:w-2/5 p-12 md:p-16 flex flex-col justify-center bg-white border-l border-gray-50">
          <div class="reveal reveal-fade-in text-left">
            <h2 class="text-4xl md:text-5xl font-black tracking-tighter leading-tight mb-6">
              <span class="bg-clip-text text-transparent bg-gradient-to-r from-blue-600 via-blue-400 to-cyan-400">{{ $videoFeature['badge'] }}</span><br>
    ...
    <div class="max-w-[1400px] mx-auto px-6">
      <div class="flex justify-between items-end mb-12">
        <div class="reveal reveal-fade-in">
          <h3 class="text-4xl md:text-5xl font-bold text-brand-ink tracking-tight">{{ $coleccion['titulo'] }}</h3>
  ...
  <section class="py-32 bg-white px-6">
    <div class="max-w-[1400px] mx-auto mb-16 px-4">
      <div class="reveal reveal-fade-in">
        <h3 class="text-4xl md:text-5xl font-bold text-brand-ink tracking-tight">{{ $soluciones['titulo'] }}</h3>
    ...
    <div class="max-w-[1400px] mx-auto grid grid-cols-2 md:grid-cols-4 gap-6">
      @foreach ($soluciones['items'] as $i => $item)
        <a href="{{ $item['url'] }}" class="aspect-square group relative overflow-hidden rounded-[40px] bg-brand-surface reveal reveal-fade-in" @if ($i > 0) style="transition-delay: {{ $i * 100 }}ms;" @endif>
          ...
          <div class="absolute inset-0 md:inset-x-0 md:inset-y-auto md:bottom-0 p-10 z-10 bg-gradient-to-t from-blue-950/90 via-blue-900/40 to-transparent flex flex-col items-center justify-center md:items-start md:justify-end w-full h-full md:h-auto">
            <p class="hidden md:block text-[10px] font-bold text-white/70 uppercase tracking-widest mb-1">{{ $item['etiqueta'] }}</p>
            <h4 class="text-3xl font-bold text-white mb-6 text-center md:text-left">{{ $item['titulo'] }}</h4>
      ...
      <div class="flex flex-col md:flex-row items-center gap-16">
        <div class="md:w-1/2 reveal reveal-fade-in">
          <div class="flex items-center gap-4 mb-8">
            ...
            <span class="text-xs font-bold uppercase tracking-[0.2em] text-green-700">{{ $eco['badge'] }}</span>
          </div>
